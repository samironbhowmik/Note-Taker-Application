...
function Home() {
    ...
    const getData = async()=>{
        ...
        const res = await axios.get("http://localhost:4000/notes", {headers})
    ...
    const handleDelete = async(id)=>{
        const headers = {"authorization":localStorage.getItem("token")}
        const res = await axios.delete(`http://localhost:4000/notes/${id}`, {headers})
        console.log(res.data);
    ...
    const handleUpdate=async(id)=>{
        // e.preventDefault()
        const headers = {"authorization":localStorage.getItem("token")}
        const res = await axios.put(`http://localhost:4000/notes/${id}`, {headers})
        console.log(res.data);
    }
